...
Radix sort is an algorithm that sorts based on the value of digits of elements within an array of integers. This can be done in parallel and has been implemented using both CUDA and MPI for our implementation. 

When we used MPI to parallelize Radix sort, the way our implementation did it was to first split the array into buckets by cutting the original array and sending each subset to a different process. Each process then sorts the array based on the way described above and then communicates to other processes to rebuild a sorted array. When we look at the data, we can see that the computation part of our algorithm is where the majority of the dividends of parallelizing our code comes from, especially when we scale strongly. As we increase the number of processes the computation time decreases. Communication on the other hand increases as we increase the number of processes until we reach about 32 processes where the time it takes to complete the communication starts to decrease. This may be explained by the fact that with 32 processes seems to be the crux where the subsets of the arrays might be the greatest but after that communication takes longer. Once we pass 32 processes, the subsets must be smaller and thus take less time. I figure it is something to do with the fact that this is strongly scaled, however it could have something to do with the hardware. Communication has a large part to play in the time it takes for the main region to run too as the graphs of the main regions largely look closer to the graphs of communication that it does to the computation graphs. Moving on weak scaling, interestingly it seems to almost follow the same patterns as the strong scaling does where computation seems to be the largest beneficiary of the increase in the number of processes. The main differences come from the communication region where the increase in number of processes sometimes has a positive and sometimes negative effect. I think this can be attributed to the fact that this is weak scaling so that there can be a different amount of elements per subset of the array. This makes intuitive sense intuitively and is a likely explanation for the better performance. Due to the performance increase from the communication, the main region also sees improvements in performance overall in the weak scaling. When it comes to the speedup for the MPI implementation of Radix sort we can see that there is a speedup, which shows that the ratio between the sequential and parallelized version is greater than 1 for most implementations. 

For CUDA, we parallelized it similarly to how we implemented the MPI version but we used threads on the GPU. The array was split up and as the subsets were being sorted the array was being updated in a shared memory. Again taking a look at the data, for the strong scaling the graph seems to be very unusual, which may be chalked up to using an incorrect implementation of Radix sort in CUDA. The graphs look all the same even though I am sure we are using the average time but I could be wrong. The thicket trees also look to be correct so it is difficult to figure why exactly this is happening. It might be because of a misconfiguration within the caliper regions, but for some graphs it seems to be working. When moving to weak scaling we see another recurrence of the same issue for some graphs and not for others. From the graphs that seem to be working, we can see that increasing the number of threads seems to have a seemingly random and menial impact on the performance of Radix sort. This again may be due to the less than ideal implementation, but it could potentially be errors in the hardware access. We can see that speedups are happening, but it is also being reported that implementing a parallel version of the code is less efficient than a sequential version. I truly believe that this is due to an error in the implementation of the algorithm because when looking at the MPI implementation, we see more stable and consistent graphs.

From our analysis of Radix sort being implemented in parallel in MPI and CUDA we can see that implementing Radix sort in parallel does have a positive effect on the performance of the algorithms.
